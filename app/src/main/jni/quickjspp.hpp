...
namespace qjs {
    ...
    namespace detail {
        ...
        class js_string : public std::string_view {
            ...
            js_string(JSContext *ctx, const char *ptr, std::size_t len) : Base(ptr, len),
                                                                          ctx(ctx) {}
...
*/
        ...
        Tuple unwrap_args_impl(JSContext *ctx, JSValue *argv, std::index_sequence<I...>) {
            return Tuple{js_traits<std::decay_t<std::tuple_element_t<I, Tuple>>>::unwrap(ctx,
                                                                                         argv[I])...};
        }

        template<typename... Args>
        std::tuple<Args...> unwrap_args(JSContext *ctx, JSValue *argv) {
            return unwrap_args_impl<std::tuple<Args...>>(ctx, argv,
        ...
        struct js_traits<fwrapper<F>> {
            static JSValue wrap(JSContext *ctx, fwrapper<F> fw) {
                return JS_NewCFunction(ctx, [](JSContext *ctx, JSValue this_value, int argc,
                    ...
                    //return JS_EXCEPTION;
                    try {
                        const auto &this_sp = js_traits<std::shared_ptr<T>>::unwrap(ctx,
                                                                                    this_value);
            ...
            static void register_class(JSContext *ctx, const char *name, JSValue proto) {
                JSClassDef def{
                        ...
                        [](JSRuntime *rt, JSValue obj) noexcept {
                            auto pptr = reinterpret_cast<std::shared_ptr<T> *>(JS_GetOpaque(obj,
                                                                                            QJSClassId));
                            if (!pptr) {
        ...
        struct js_property_traits<const char *> {
            static void
            set_property(JSContext *ctx, JSValue this_obj, const char *name, JSValue value) {
            ...
            static JSValue
            get_property(JSContext *ctx, JSValue this_obj, const char *name) noexcept {
        ...
        struct js_property_traits<uint32_t> {
            static void
            set_property(JSContext *ctx, JSValue this_obj, uint32_t idx, JSValue value) {
        ...
        struct property_proxy {
            ...
            operator Value() const {
                return js_traits<Value>::unwrap(ctx,
                                                js_property_traits<Key>::get_property(ctx, this_obj,
                                                                                      key));
            }
    ...
    public:
        ...
        Value eval(std::string_view buffer, const char *filename, unsigned eval_flags = 0) {
            ...
            if (JS_IsException(v))
                js_print(ctx, JS_NULL, 1, (JSValueConst *) &v);
            if (JS_IsError(ctx, v)) {
                val = JS_GetPropertyStr(ctx, v, "stack");
        ...
        Value evalFile(const char *filename, unsigned eval_flags = 0) {
            ...
            auto buf = std::unique_ptr<uint8_t, decltype(deleter)>{
                    js_load_file(ctx, &buf_len, filename), deleter};
            if (!buf)
